...
int main()
{
	...
	if (hModule == NULL) {
		WARN("Load Libarary failed!! error : %d", GetLastError());
		return 1;
	}

	pAddress = GetProcAddress(hModule, SACRIFICIAL_FUNC);
	if (pAddress == NULL) {
		WARN("Error GetProcAddress error : %d", GetLastError());
		return 1;
	}
	...
	{
		WARN("Local Stomping Injection Failed! Reason: %lu", GetLastError());
		return 1;
	}
