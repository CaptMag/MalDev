#include "box.h"

int main()
{

	UCHAR Shellcode[] = {
	
	};

	SIZE_T SizeOfShellcode = sizeof(Shellcode);
	CHAR Key = "Z";

	INFO("Original Data");
	for (int i = 0; i < SizeOfShellcode; i++)
	{
		if (i % 16 == 0)
		{
			printf("\n ");
		}
		printf(" %02x", Shellcode[i]);
	}
	puts("\n");

	INFO("Encrypting via XOR");
	CHAR("Encrypt Shellcode..");
	getchar();

	if (!XorEncrypt(Shellcode, SizeOfShellcode, Key))
	{
		PRINT_ERROR("XorEncrypt");
		return 1;
	}

	INFO("Decrypting XOR Shellcode");
	CHAR("Decrypt Shellcode...");
	getchar();

	if (!XorDecrypt(Shellcode, SizeOfShellcode, Key))
	{
		PRINT_ERROR("XorDecrypt");
		return 1;
	}

	OKAY("DONE!");

	CHAR("Quit...");
	getchar();

	return 0;

}