#include "box.h"

int main()
{

	UCHAR Shellcode[] = {
	
	};

	SIZE_T SizeOfShellcode = sizeof(Shellcode);

	INFO("Original Data");
	for (int i = 0; i < SizeOfShellcode; i++)
	{
		if (i % 16 == 0)
		{
			printf("\n ");
		}
		printf(" %02x", Shellcode[i]);
	}
	puts("\n");

	INFO("Encrypting via RC4");
	CHAR("Encrypt Shellcode..");
	getchar();

	if (!Rc4Encrypt(Shellcode, SizeOfShellcode))
	{
		PRINT_ERROR("Rc4Encrypt");
		return 1;
	}

	INFO("Decrypting RC4 Shellcode");
	CHAR("Decrypt Shellcode...");
	getchar();

	if (!Rc4Decrypt(Shellcode, SizeOfShellcode))
	{
		PRINT_ERROR("Rc4Decrypt");
		return 1;
	}

	OKAY("DONE!");

	CHAR("Quit...");
	getchar();

	return 0;

}