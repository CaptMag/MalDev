#include "box.h"

int main()
{

	UCHAR Shellcode[] = {
	
	};

	SIZE_T SizeOfShellcode = sizeof(Shellcode);

	PBYTE EncryptedData, DecryptedData, Tag;
	DWORD EncryptedSize, DecryptedSize;

	INFO("Original Data");
	for (int i = 0; i < SizeOfShellcode; i++)
	{
		if (i % 16 == 0)
		{
			printf("\n ");
		}
		printf(" %02x", Shellcode[i]);
	}
	puts("\n");

	INFO("Original Size: %zu Bytes", SizeOfShellcode);

	INFO("Encrypting via AES-256-GCM");
	CHAR("Encrypt Data...");
	getchar();
	
	if ((EncryptedData = (PBYTE)AesEncrypt(Shellcode, SizeOfShellcode, &EncryptedSize, &Tag)) == NULL)
	{
		PRINT_ERROR("AesEncrypt");
		return 1;
	}

	OKAY("Successfully Encrypted!");

	INFO("Encrypted Data");
	for (int i = 0; i < EncryptedSize; i++)
	{
		if (i % 16 == 0)
		{
			printf("\n ");
		}
		printf(" %02x", EncryptedData[i]);
	}
	puts("\n");

	INFO("Encrypted Size: %zu Bytes", EncryptedSize);

	INFO("Decrypting Aes-256-GCM");
	CHAR("Decrypt Data...");
	getchar();

	if ((DecryptedData = (PBYTE)AesDecrypt(EncryptedData, EncryptedSize, Tag, 16, &DecryptedSize)) == NULL)
	{
		PRINT_ERROR("AesDecrypt");
		return 1;
	}

	OKAY("Decrypted Successfully!");

	INFO("Decrypted Data");
	for (int i = 0; i < DecryptedSize; i++)
	{
		if (i % 16 == 0)
		{
			printf("\n ");
		}
		printf(" %02x", DecryptedData[i]);
	}
	puts("\n");

	INFO("Decrypted Size: %d Bytes", DecryptedSize);

	CHAR("Quit...");
	getchar();

	return 0;
}