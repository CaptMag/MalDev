...
int main()
{
    ...
    INFO("Running APC Injection...");

    if (!ApcInject(hProcess, hThread, Shellcode, shellsize))
    {
        WARN("Failed to Inject via APC! Reason %lu", GetLastError());
        return 1;
    }
    ...
    OKAY("Successfully Injected via APC Injection!");
